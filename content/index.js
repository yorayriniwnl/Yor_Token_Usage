(() => {
  ...
  function roleHintFromNode(node, fallbackIndex = 0) {
    ...
    if (/\b(assistant|model|ai|response|answer)\b|font-claude|assistant-message|ai-message/.test(haystack)) return "assistant";
  ...
  var MAX_COLLECTED_MESSAGES = 240;
  var SelectorSiteAdapter = class {
    ...
    collectMessages() {
      ...
      return messages.sort((a, b) => a.node.compareDocumentPosition(b.node) & Node.DOCUMENT_POSITION_PRECEDING ? 1 : -1).map(({ node, ...message }) => message).slice(-MAX_COLLECTED_MESSAGES);
    }
    getModelName() {
  ...
  var claudeAdapter = new SelectorSiteAdapter({
    ...
    userMessage: ['[data-testid="user-message"]', '[data-testid*="user-message"]', 'div[data-is-author="human"]', '[data-is-author="human"]', 'div[data-is-author="user"]', '[data-is-author="user"]', '[data-testid*="human-message"]', '[class*="font-user-message"]'],
...
` : line;
  ...
  // Image token calculation using provider tile formulas
  function imageTokensFromDimensions(width, height, model) {
    // GPT-4o / GPT-4.1 tile formula: 85 base + 170 per 512×512 tile
    if (!model || /gpt|openai|o[34]/.test(model)) {
      const tiles = Math.ceil(width / 512) * Math.ceil(height / 512);
      return 85 + 170 * tiles;
    }
    // Claude: approximately 1.6 tokens per pixel normalized to 1568px long edge
    const longEdge = Math.max(width, height);
    const scale = longEdge > 1568 ? 1568 / longEdge : 1;
    const scaledW = Math.round(width * scale);
    const scaledH = Math.round(height * scale);
    return Math.ceil(scaledW * scaledH / 750);
  }
  function describeAttachments(attachments) {
    return attachments.map((attachment, index) => {
      ...
      // Use proper image tile formula when dimensions are available
      const isImage = /\.(png|jpe?g|gif|webp|bmp)$/i.test(attachment.name ?? "")
        || /^image\//.test(attachment.mimeType ?? "");
      let tokens;
      if (isImage && attachment.width && attachment.height) {
        tokens = imageTokensFromDimensions(attachment.width, attachment.height, null);
      } else if (attachment.pages) {
        // PDF: ~1300 tokens per page (typical full-page text)
        tokens = Math.ceil(attachment.pages * 1300 + 50);
      } else if (attachment.sizeBytes) {
        // Generic: bytes / 4 as rough character → token proxy, capped
        tokens = Math.min(Math.ceil(attachment.sizeBytes / 4), 32000);
      } else {
        tokens = Math.ceil(description.length / 7 + 12);
      }
      return {
  ...
  function estimateConversation(messages) {
    // Correct cumulative accounting: each API call sends the full conversation
    // history, so input tokens for turn N = sum of all prior messages + turn N.
    // We track running context size, not per-message size.
    let promptTokens = 0;
    let outputTokens = 0;
    ...
    for (const message of messages) {
      ...
      if (message.role === "assistant") {
        ...
        // User message is added to context before the API call
        runningContext += tokens;
        promptTokens += runningContext; // full context sent as input each turn
      }
    ...
    return {
      promptTokens,
      outputTokens,
      totalTokens: runningContext,
      contextGrowth
    };
  }
  function conversationFromStoredThread(thread) {
    if (!thread || typeof thread !== "object") return void 0;
    const promptTokens = Math.max(0, Number(thread.promptTokens) || 0);
    const outputTokens = Math.max(0, Number(thread.outputTokens) || 0);
    const totalTokens = Math.max(0, Number(thread.totalTokens) || 0);
    if (promptTokens <= 0 && outputTokens <= 0 && totalTokens <= 0) return void 0;
    return {
      promptTokens,
      outputTokens,
      totalTokens: totalTokens > 0 ? totalTokens : promptTokens + outputTokens,
      contextGrowth: Array.isArray(thread.contextGrowth) ? thread.contextGrowth.filter(Number.isFinite).map((value) => Math.max(0, value)).slice(-25) : []
    };
  }
  function preferStoredConversation(domConversation, storedThread) {
    const storedConversation = conversationFromStoredThread(storedThread);
    if (!storedConversation) return domConversation;
    const storedSignal = storedConversation.promptTokens + storedConversation.outputTokens;
    const domSignal = domConversation.promptTokens + domConversation.outputTokens;
    return storedSignal > domSignal || storedConversation.totalTokens > domConversation.totalTokens ? storedConversation : domConversation;
  }
...
${structured.output.slice(0, outputLimit).map((line) => `- ${line}`).join("\n")}`);
  ...
  var MODEL_COST_RATES = {
    ...
    o3: { input: 2e-3, output: 8e-3 },
    "o3-mini": { input: 11e-4, output: 44e-4 },
    "o4-mini": { input: 11e-4, output: 44e-4 },
    "claude-opus-4.5": { input: 0.015, output: 0.075 },
    "claude-opus-4.1": { input: 0.015, output: 0.075 },
    "claude-opus-3": { input: 0.015, output: 0.075 },
    "claude-sonnet": { input: 3e-3, output: 0.015 },
    "claude-sonnet-4.6": { input: 3e-3, output: 0.015 },
    "claude-haiku-4.5": { input: 1e-3, output: 5e-3 },
    "claude-haiku-3.5": { input: 8e-4, output: 4e-3 },
    "gemini-pro": { input: 125e-5, output: 0.01 },
    "gemini-2.5-flash": { input: 3e-4, output: 25e-4 },
    "gemini-2.5-flash-lite": { input: 1e-4, output: 4e-4 },
    "gemini-2.0-flash": { input: 1e-4, output: 4e-4 },
    "gemini-2.0-flash-lite": { input: 75e-6, output: 3e-4 },
    "sonar-pro": { input: 3e-3, output: 0.015 },
    "grok-3": { input: 3e-3, output: 0.015 },
    "grok-3-mini": { input: 3e-4, output: 5e-4 }
  };
  var MODEL_COST_LOOKUP = Object.fromEntries(Object.keys(MODEL_COST_RATES).map((id) => [normalizeModelKey(id), id]));
  var SITE_MODEL_COST_FALLBACKS = {
    chatgpt: "gpt-4o",
  ...
  function resolveModelCostRate(model, site = "generic") {
    const key = normalizeModelKey(model);
    const exactId = MODEL_COST_LOOKUP[key];
    if (exactId) return MODEL_COST_RATES[exactId];
    for (const [pattern, id] of MODEL_CONTEXT_MATCHERS) {
      if (pattern.test(key) && MODEL_COST_RATES[id]) return MODEL_COST_RATES[id];
    }
    return MODEL_COST_RATES[SITE_MODEL_COST_FALLBACKS[site] ?? "generic"] ?? MODEL_COST_RATES.generic;
  }
  ...
  function formatCurrency(value) {
    ...
    const fractionDigits = safeValue > 0 && safeValue < 0.01 ? 4 : 2;
    return new Intl.NumberFormat(void 0, {
      style: "currency",
      currency: "USD",
  ...
  function activeLengthTokens(state) {
    ...
    const conversationTokens = Math.max(0, (conversation.promptTokens ?? 0) + (conversation.outputTokens ?? 0), conversation.totalTokens ?? 0);
    return conversationTokens + (state.currentInput?.trim() ? state.analysis.inputTokens : 0);
  }
  function activeCostUsd(state) {
    const rates = resolveModelCostRate(state.model, state.site);
    const draftInput = state.currentInput?.trim() ? state.analysis.inputTokens : 0;
    const inputTokens = (state.conversation?.promptTokens ?? 0) + draftInput;
    const outputTokens = state.conversation?.outputTokens ?? 0;
...
`;
  var OverlayWidget = class {
    ...
    mountShell() {
      ...
      <div class="yor-root" data-ref="root">
        <button class="yor-page-meter" data-action="toggle" data-ref="pageMeter" title="Open Yor Token Usage details">
          ...
          <span>| Cost: <strong data-ref="meterCost"></strong></span>
        </button>
    ...
    render(state) {
      ...
      const costEstimate = activeCostUsd(state);
      ...
      this.setText("meterCost", formatCurrency(costEstimate));
      ...
      this.setText("quickCost", `Cost: ${formatCurrency(costEstimate)}`);
      ...
      this.setText("threadNote", messages.length >= MAX_COLLECTED_MESSAGES ? `${formatInteger(messages.length)} messages sampled` : `${messages.length} messages detected`);
      ...
      this.setText("conversationPressure", `${formatTokens(state.conversation.promptTokens)} input / ${formatTokens(state.conversation.outputTokens)} output`);
  ...
  async function init() {
    ...
    const renderOverlayState = (session, conversation, messages, snapshot = snapshotCache) => {
      ...
      const lastEvent = snapshot?.analytics?.timeline?.find((event) => event.site === adapter.site && event.threadId === threadId) ?? snapshot?.analytics?.timeline?.find((event) => event.site === adapter.site) ?? snapshot?.analytics?.timeline?.[0];
      latestOverlayState = {
        site: session.site,
        model: session.model,
    ...
    const syncSession = async () => {
      ...
      const domConversation = estimateConversation(messages);
      const storedSession = stateCache?.sessions[adapter.site];
      const storedThread = storedSession?.threadId === adapter.getThreadId() ? storedSession.currentThread : void 0;
      const conversation = preferStoredConversation(domConversation, storedThread);
      ...
      overlay.setVisible(overlayEnabled);
      const siteEvents = stateCache?.usageEvents.filter((event) => event.site === adapter.site) ?? [];
    ...
    // ── Network token listener ───────────────────────────────────────────────
    // The MAIN-world interceptor dispatches real token counts via CustomEvent.
    // We forward them to the service worker which patches the session state.
    window.addEventListener("YOR_NETWORK_TOKENS", (ev) => {
      const { site, tokenData, headers, timestamp } = ev.detail ?? {};
      if (!site) return;
      const promptPreview = pendingPrompt?.prompt ? truncate(pendingPrompt.prompt, 140) : truncate(adapter.readComposerText(), 140);
      if ((tokenData?.inputTokens ?? 0) > 0 || (tokenData?.outputTokens ?? 0) > 0) {
        pendingPrompt = void 0;
      }
      sendRuntimeMessage({
        type: "network-token-data",
        payload: {
          site,
          tokenData,
          headers,
          timestamp,
          threadId: adapter.getThreadId(),
          model: tokenData?.model ?? headers?.detectedModel ?? adapter.getModelName(),
          promptPreview
        }
      }).then((response) => {
        if (!response?.snapshot?.state) return;
        snapshotCache = response.snapshot;
        stateCache = response.snapshot.state;
        preferences = response.snapshot.state.preferences ?? preferences;
        const session = stateCache.sessions[adapter.site];
        if (!session) return;
        const messages = adapter.collectMessages();
        const domConversation = estimateConversation(messages);
        const storedThread = session.threadId === adapter.getThreadId() ? session.currentThread : void 0;
        renderOverlayState(session, preferStoredConversation(domConversation, storedThread), messages, response.snapshot);
      }).catch(() => void 0);
    });

    // ── Soft-navigation re-init ──────────────────────────────────────────────
    // SPAs replace DOM nodes without a full page load. Re-bind when the
    // document title changes (cheapest reliable soft-nav signal).
    let lastTitle = document.title;
    let lastConvRoot = adapter.getConversationRoot();
    const titleObserver = new MutationObserver(() => {
      const newTitle = document.title;
      if (newTitle !== lastTitle) {
        lastTitle = newTitle;
        const newRoot = adapter.getConversationRoot();
        if (newRoot !== lastConvRoot) {
          lastConvRoot = newRoot;
          observer.disconnect();
          observer.observe(newRoot ?? document.body, { childList: true, subtree: true, characterData: true });
        }
        scheduleSync(400);
      }
    });
    titleObserver.observe(document.querySelector("title") ?? document.head, { childList: true, characterData: true, subtree: true });

    // ── Adapter health check ─────────────────────────────────────────────────
    // Detect silent DOM adapter failures. If messages = 0 and we know a
    // conversation should exist (URL has a thread ID), warn visually.
    let healthCheckCount = 0;
    const checkAdapterHealth = () => {
      healthCheckCount++;
      if (healthCheckCount < 3) return; // give page time to load
      const messages = adapter.collectMessages();
      const hasThread = window.location.pathname.split("/").filter(Boolean).length > 1;
      if (messages.length === 0 && hasThread) {
        const composerOk = Boolean(adapter.getComposer());
        if (!composerOk) {
          // Selector failure — show warning badge on overlay if visible
          overlay?.setHealthWarning?.("Adapter selectors may be stale. Token counts unavailable.");
        }
      } else {
        overlay?.clearHealthWarning?.();
      }
    };

    // ── Live composer cost counter ───────────────────────────────────────────
    // Update the overlay with live token + cost estimate on every keystroke.
    const updateComposerCounter = () => {
      const text = adapter.readComposerText?.() ?? "";
      if (!text.trim()) { overlay?.setComposerTokens?.(0, 0); return; }
      const breakdown = estimateTokenBreakdown(text);
      const model = adapter.getModelName?.() ?? stateCache?.sessions[adapter.site]?.model ?? "generic";
      const rates = resolveModelCostRate(model, adapter.site);
      const inputCost = breakdown.total / 1e3 * rates.input;
      overlay?.setComposerTokens?.(breakdown.total, inputCost);
    };
    const debouncedComposerUpdate = debounce(updateComposerCounter, 120);

    // Patch composer event binding to also update live counter
    const _origBindLiveControls = bindLiveControls;
    const patchedBind = () => {
      _origBindLiveControls?.();
      const composer = adapter.getComposer();
      if (composer) composer.addEventListener("input", debouncedComposerUpdate, { passive: true });
    };
    try { patchedBind(); } catch { /* ignore if bindLiveControls not accessible */ }
