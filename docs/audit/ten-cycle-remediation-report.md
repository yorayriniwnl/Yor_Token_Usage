...
GitHub Actions run #33 passed both backend and extension jobs. The report-only image scan kept all three high advisories visible in its logs: CVE-2026-85091, CVE-2026-19499, and CVE-2026-5435. The blocking scan then returned `No vulnerabilities found` with `only-fixed: true`, so the build is gated on actionable high/critical findings while the unpatched Debian 13 advisories remain disclosed above. The backend verify, migration, seed, integration, Docker build, Chromium fixtures, capture flow, normal-motion layout, and package validation all passed. A local YAML parse confirmed the two scan steps and their settings; `git diff --check` passed. This Windows host still has no Docker engine, so the image build and both scans were verified on the GitHub runner.
