...
function renderMetrics(snapshot) {
  ...
  const totalCost = (Array.isArray(snapshot.analytics.byDay) ? snapshot.analytics.byDay : []).reduce((a, d) => a + safeNumber(d.cost), 0);
  const exactEvents = (Array.isArray(snapshot.analytics.timeline) ? snapshot.analytics.timeline : []).filter((e) => e.accuracy === "exact").length;
  const totalEvents = (Array.isArray(snapshot.analytics.timeline) ? snapshot.analytics.timeline : []).length;
  const exactPct = totalEvents > 0 ? Math.round(exactEvents / totalEvents * 100) : 0;
  metrics.innerHTML = [
    { label: "Cost today", value: formatCurrency(snapshot.analytics.byDay?.at(-1)?.cost ?? 0), primary: true },
    { label: "Cost 7 days", value: formatCurrency(snapshot.summary.costThisWeek), primary: true },
    { label: "Tokens today", value: formatTokens(snapshot.summary.tokensToday) },
    ...
    { label: "Exact readings", value: `${exactPct}%` },
    ...
    (item) => `
        <article class="metric-card${item.primary ? " metric-primary" : ""}">
...
async function render() {
  ...
  timeline.innerHTML = timelineItems.length ? timelineItems.map(
    (event) => {
      const accLabel = event.accuracy === "exact" ? "Exact" : event.accuracy === "inferred" ? "Inferred" : "Est.";
      const accCls = event.accuracy === "exact" ? "badge-exact" : event.accuracy === "inferred" ? "badge-inferred" : "badge-est";
      const cost = Number.isFinite(event.costUsd) ? formatCurrency(event.costUsd) : "";
      const cacheLine = event.cacheReadTokens > 0 ? ` \u2022 ${formatTokens(event.cacheReadTokens)} cached` : "";
      return `
            <div class="timeline-item">
              <div class="tl-head">
                <strong>${escapeHtml(`${SITE_LABELS[event.site] ?? event.site} \u00b7 ${event.model}`)}</strong>
                <span class="accuracy-badge ${accCls}">${escapeHtml(accLabel)}</span>
              </div>
              <p>${escapeHtml(truncate(event.promptPreview, 180))}</p>
              <small>${escapeHtml(`${formatDateTime(event.timestamp)} \u00b7 ${formatTokens(event.totalTokens)} tokens${cacheLine}${cost ? " \u00b7 " + cost : ""} \u00b7 ${event.status}`)}</small>
            </div>
          `;
    }
