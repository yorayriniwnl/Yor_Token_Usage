...
function eventCost(event, preferences) {
  // Prefer cost snapshotted at write time — prevents wrong numbers if pricing changes
  if (Number.isFinite(event.costUsd) && event.costUsd >= 0) return event.costUsd;
...
function computeEventCostUsd(promptTokens, outputTokens, model, site) {
  const catalog = resolveModelProfile(model, site);
  const inputRate = catalog.estimatedInputCostPer1k ?? 0;
  const outputRate = catalog.estimatedOutputCostPer1k ?? 0;
  return promptTokens / 1e3 * inputRate + outputTokens / 1e3 * outputRate;
}
function normalizeUsageEvent(event) {
  ...
  const model = stringOr(event.model, "generic");
  const site = normalizeSite(event.site);
  // Snapshot cost at write time — never recalculate from stored events
  const costUsd = Number.isFinite(event.costUsd) ? event.costUsd : computeEventCostUsd(promptTokens, outputTokens, model, site);
  const cacheReadTokens = nonNegativeNumberOr(event.cacheReadTokens);
  const cacheCreateTokens = nonNegativeNumberOr(event.cacheCreateTokens);
  const modelProfile = resolveModelProfile(model, site);
  const cacheSavingsUsd = cacheReadTokens > 0
    ? cacheReadTokens / 1e3 * ((modelProfile.estimatedInputCostPer1k ?? 0) * 0.9) : 0;
  return {
    ...event,
    site,
    model,
    ...
    cacheCreateTokens,
    cacheSavingsUsd,
    accuracy: event.accuracy === "exact" ? "exact" : event.accuracy === "inferred" ? "inferred" : "estimated",
...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  chrome.alarms.create("yor-keepalive", { periodInMinutes: 0.4 });
  await updateBadge();
});
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  if (alarm.name === "yor-keepalive") {
    // No-op storage ping to prevent service worker termination during active sessions
    await storageGet(STATE_KEY).catch(() => void 0);
    return;
  }
  ...
  void (async () => {
    try {
      switch (message?.type) {
        ...
        case "network-token-data": {
          // Real token counts from the MAIN world network interceptor.
          // Patch the active session for this site with exact numbers and
          // update quota from real rate-limit headers if present.
          const { site, tokenData, headers, timestamp, threadId, model, promptPreview } = message.payload ?? {};
          const normalizedSite = normalizeSite(site);
          if (normalizedSite !== "generic" && (tokenData || headers)) {
            const eventTimestamp = Number.isFinite(timestamp) ? timestamp : Date.now();
            const cacheReadTokens = nonNegativeNumberOr(tokenData?.cacheRead);
            const cacheCreateTokens = nonNegativeNumberOr(tokenData?.cacheCreate);
            const exactInputTokens = nonNegativeNumberOr(tokenData?.inputTokens) + cacheReadTokens + cacheCreateTokens;
            const exactOutputTokens = nonNegativeNumberOr(tokenData?.outputTokens);
            const hasExactTokens = exactInputTokens > 0 || exactOutputTokens > 0;
            let updatedState = await updateState(async (state) => {
              const existingSession = state.sessions[normalizedSite];
              const detectedModel = stringOr(tokenData?.model) || stringOr(headers?.detectedModel) || stringOr(model) || stringOr(existingSession?.model, "generic");
              const normalizedThreadId = stringOr(threadId, stringOr(existingSession?.threadId, "default"));
              const session = existingSession ?? {
                site: normalizedSite,
                model: detectedModel,
                threadId: normalizedThreadId,
                currentInput: "",
                currentEstimate: defaultPromptAnalysis(),
              ...
              const updatedSession = {
                ...
                model: detectedModel,
                threadId: normalizedThreadId,
                adapterConfidence: hasExactTokens ? 1 : session.adapterConfidence
              };
              if (hasExactTokens) {
                const previousThread = session.currentThread ?? {};
                const contextTotal = exactInputTokens + exactOutputTokens;
                updatedSession.currentThread = {
                  ...previousThread,
                  threadId: normalizedThreadId,
                  site: normalizedSite,
                  model: detectedModel,
                  messageCount: nonNegativeNumberOr(previousThread.messageCount) + (exactOutputTokens > 0 ? 2 : 1),
                  promptTokens: exactInputTokens,
                  outputTokens: exactOutputTokens,
                  totalTokens: contextTotal,
                  accuracy: "exact",
                  lastUpdated: eventTimestamp,
                  contextGrowth: [
                    ...(Array.isArray(previousThread.contextGrowth) ? previousThread.contextGrowth.filter(Number.isFinite).map((value) => Math.max(0, value)).slice(-24) : []),
                    contextTotal
                  ]
                };
              }
              // Upgrade quota from real rate-limit headers
              if (Number.isFinite(headers?.rateLimitTokensRemaining)) {
                updatedSession.quota = {
                  ...session.quota,
                  remainingTokens: headers.rateLimitTokensRemaining,
                  accuracy: "exact",
                  status: headers.rateLimitTokensRemaining < 500 ? "warning" : "ok"
                };
                if (headers.rateLimitResetAt) {
                  updatedSession.quota.nextReset = {
                    resetAt: headers.rateLimitResetAt,
                    localLabel: new Date(headers.rateLimitResetAt).toLocaleTimeString(void 0, { hour: "numeric", minute: "2-digit" }),
                    kind: "exact"
                  };
                }
              }
              updatedSession.lastUpdated = eventTimestamp;
              return { ...state, sessions: { ...state.sessions, [normalizedSite]: updatedSession } };
            });
            let savedEvent;
            if (hasExactTokens) {
              const session = updatedState.sessions[normalizedSite];
              const event = {
                id: `net:${normalizedSite}:${session?.threadId ?? threadId ?? "default"}:${eventTimestamp}:${exactInputTokens}:${exactOutputTokens}`,
                site: normalizedSite,
                model: session?.model ?? model ?? "generic",
                threadId: session?.threadId ?? threadId ?? "default",
                timestamp: eventTimestamp,
                promptTokens: exactInputTokens,
                outputTokens: exactOutputTokens,
                totalTokens: exactInputTokens + exactOutputTokens,
                promptChars: stringOr(promptPreview).length,
                outputChars: 0,
                status: "completed",
                accuracy: "exact",
                promptPreview: stringOr(promptPreview).slice(0, 140),
                cacheReadTokens,
                cacheCreateTokens
              };
              ...
              if (result.recorded && result.event) {
                await notifyIfNeededFromEvent(result.event);
              }
            }
            await updateBadge(updatedState.sessions[normalizedSite]);
            sendResponse({ ok: true, snapshot: buildSnapshot(updatedState), event: savedEvent });
          } else {
